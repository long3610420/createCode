...
list_action
	...
	try {
        ...
        request.setAttribute("consumeDate2", sdf.format(calendar.getTime()));
        calendar.add(Calendar.DATE, -1);
        request.setAttribute("consumeDate", sdf.format(calendar.getTime()));
	 /**
	  * 向前台发出成功失败跳转页面
	  */ 
	    
	    if (OMSConstants.SUCESS_RET_CODE.equals(retCodex)) {
	    	String retmsg = respDto.getRetMsg();
	    	request.getSession().setAttribute(MVCDRILLConstants.ERROR_MSG_DRILL, retmsg);
